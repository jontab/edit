...
class TermboxBackend : public IViewBackend
{
  public:
    ...
    void poll(ActionBus &action_bus, const std::function<void(unsigned int, unsigned int)> &on_resize) override;

  private:
    void on_key(const struct tb_event &ev, ActionBus &action_bus);
};
